...
SKY.Carousel.prototype = {
	...
	initDOM: function() {
		...
		this.dom.images = this.dom.container.find('li div.item');
	...
	getStartItem: function() {
		...
		this.currentPointer = (index * this.settings.itemWidth) + (this.settings.itemWidth / 2)
	...
	calculateUnits: function() {
		...
		this.minAlphaUnit = (1 - s.minAlpha) / (s.itemWidth/3);
	...
	update: function() {
		...
		var self = this,
			...
			containerLeft = container.getLeft(),
		 	zSortList = [],
		 	idx = (this.closestItem) ? this.closestItem.index() : this.selectedItem.index();
		 	
		var	ref = self.currentPointer;
		if(ref > this.maxP) { 
			ref = this.maxP 
		};
		if(ref < this.minP) {
		...
		for (var i = 0; i < this.carouselItems.length; i++) {
			var item = this.carouselItems[i],
				distance = (containerLeft + item.x + s.itemWidth / 2) - ref,
			...
			if (absDistance <= s.motionStartDistance) {
				var alpha = this.calculateAlpha(absDistance),
					...
					extraDistance = (i == idx) ? 0 : this.calculateExtraDistance(absDistance);
			} else {
			...
			if (absDistance <= ref + s.itemWidth || this.clickSelect) {
				// Add the items within the motion start distance to the zSortList and at the start sort all once (if closestItem == null)
			...
			if(i == idx) {
				var txtAlpha = (this.clickSelect) ? 1 : this.calculateMinAlpha(absDistance);
				this.contentContainer.find(".sc-content").css("opacity", txtAlpha);
			
				// Add the .sc-selected class to the current item and set correct x for sc-content div and hide correct vline
				item.addClass('sc-selected');
				var l = item.x;
				if(i >= 3) {
					//if current highlighted item is the center one the text div floats to the current - 1 otherwise current - 2
					var refIdx = (i==3 ? (i-1) : (i-2));
					var refItem = this.carouselItems[refIdx];
					var targetWidth = refItem.actualWidth * refItem.scale,
					l = refItem.x + (refItem.actualWidth - targetWidth) / 2;
				}
				this.contentContainer.find(".sc-content").css("left", l+"px");
			} else {
				item.removeClass('sc-selected');
			}
			...
			//vline floating or invisible depending on idx
			var float = "left";
			var v ="visible";
			if(i > idx){
				float="right";
			...
			item.vline.css("float", float);
			item.circle.css("float", float);
			item.vline.css("visibility", v);
			// Help garbage collector
			distance = absDistance = alpha = scale = extraDistance = float = v =  null;
		}
	...
	updatePlugin: function() {
		...
		this.offset = this.dom.carousel.position().left;
		this.minP = this.settings.itemWidth / 2;
		this.maxP = (this.carouselItems.length - 1) * (this.settings.itemWidth * this.settings.unselectedItemZoomFactor + this.settings.distance) + this.minP;
		this.updateLayout();
	},
	 ...
	 * @return {Number} Target alpha value for the given distance.
	 */
	calculateMinAlpha: function(absDistance) {
		
		return (this.settings.itemWidth/3 - absDistance) * 
			this.minAlphaUnit + 
	...
	onStart: function(evt) {
		...
		var self = this,
			...
			currentPointer = lastX - this.offset;
		...
		//self.dom.container.find('.vline').addClass('hidden');
		...
		function onMove(evt) {
			...
			vx = lastX - pointerX;
			lastX = pointerX;
			self.currentPointer = pointerX - self.offset;
			// If there are more than 1 touch points, let the browser do the default action
			if (SKY.Utils.hasTouchSupport()) {
				if (oEvt.touches.length > 1) {
					completeDefaultAction = true;
		...
		function onEnd(evt) {
			...
			//self.dom.container.find('.vline').removeClass('hidden');
			...
			if (Math.abs(startX - endX) == 0) {
				...
				if (element.hasClass('sc-image')) {
					...
					if (s.selectByClick && item !== self.selectedItem) {
						...
						self.currentPointer = item.x + s.itemWidth / 2;
				...
				index += slideCount;
//				index = Math.round(self.currentPointer / self.settings.itemWidth)
				// Bound check
